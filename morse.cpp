...
Morse::node *Morse::add_node(node *temp, string code, char c, const string morse_code)
{
    ...
    if (code.length() == 0) //if there are no more . or _
    {
        ...
        temp->code = morse_code; //set code2 (morse code translation) into current node code
        return temp;
    }
    else
    {
        if (code[0] == DOT)                                                     //if next char is a DOT
            temp->left = add_node(temp->left, code.substr(1), c, morse_code);   //go left and recursively call
        else                                                                    //if next char is a DASH
            temp->right = add_node(temp->right, code.substr(1), c, morse_code); //go right and recursively call
    }
    return temp;
}
...
{
    if (isalpha(message[0]) == 0)                                 //if the character is not a letter of the alphabet
        encode(root, message = message.substr(1), code += " ? "); //add a ? since the char doesnt exist
    if (message[0] == temp->letter)                               //if the letter is found
    {
        ...
        if (message.substr(1).length() != 0)                          //if not at the last letter
            encode(root, message = message.substr(1), code += SPACE); //recursive call where the message loses the first char, and a space is added to code
        return;
    }
